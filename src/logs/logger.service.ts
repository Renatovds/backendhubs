...
export class Logger {
  ...
  async wrFile(data: DataHub[]) {
    const cachedData = await this.hubsCached.getValueHubs('hubs');

    for (const hub of data) {
      const hubLogCache = await this.hubsCached.getValueLog(hub.name);
      ...
      if (oldHubData.error && !hub.error) {
        const cachedProblemsHub = await this.hubsCached.getValueLog(hub.name);
        ...
        const indexItemDelete = hubLogCache.findIndex((obj) => obj._id == _id);
      ...
      for (const task of hub.tasks) {
        if (!oldHubData.tasks.some((e) => e.TarefaId === task.TarefaId)) {
          const log = await this.logsService.create({
            hub: hub.name,
            taskNumber: task.TarefaId,
            taskName: task.Nome,
            status: `${task.Situacao === '0' ? 'Parada' : 'Atrasada'}`,
      ...
      for (const t of oldHubData.tasks) {
        if (!hub.tasks.some((e) => e.TarefaId === t.TarefaId)) {
          const cachedProblemsTasksHub = await this.hubsCached.getValueLog(
            hub.name,
          );
